export default function ContinueReading({ stories, progress, bookmarks = {}, onStoryClick, language }) {
  ...
  // Filter stories that have progress, matching by string/number
  const myBooks = stories.filter((s) => {
    const storyId = String(s.id);
    return progress[storyId] || progress[s.id] || bookmarks[storyId] || bookmarks[s.id];
  });
    ...
    <div className="p-6 space-y-6 pb-20 animate-fade-in bg-background min-h-screen text-foreground">
      ...
      {myBooks.length === 0 ? (
        ...
        myBooks.map(story => {
          ...
          const prog = progress[storyId] || progress[story.id];
          return (
          <div
            key={story.id}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] text-muted-foreground mt-1 line-clamp-2">
                {prog?.last_scene_summary || story.description}
              </p>
